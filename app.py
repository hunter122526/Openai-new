...
def create_app():
    ...
    # Configure Flask-WTF web allow hosts to include ngrok domains
    app.config["WTF_CSRF_CHECK_DEFAULT"] = False  # Disable default CSRF check for web forms
    app.config["WTF_CSRF_SSL_STRICT"] = False  # Allow CSRF over non-HTTPS for development
    # Allow ngrok domains and localhost
    app.config["WTF_CSRF_ALLOWED_HOSTS"] = [
        "127.0.0.1",
        "localhost",
        "*.ngrok-free.dev",
        "*.ngrok.app",
        "vernia-noncreeping-michiko.ngrok-free.dev"
    ]

    # Configure Flask-WTF web allow hosts (this is the correct config key)
    app.config["WTF_CSRF_WEB_ALLOW_HOSTS"] = [
        "127.0.0.1",
        "localhost",
        "*.ngrok-free.dev",
        "*.ngrok.app",
    ...
    # Configure Flask allowed hosts for security
    app.config["SERVER_NAME"] = None  # Allow all hosts in development
    app.config["PREFERRED_URL_SCHEME"] = "http" if not USE_HTTPS else "https"
    # Disable Flask's host header validation for ngrok
